...
# @Last Modified time: sz 2018-10-10 23:05:26 
# sz python 3.6.3 torch 0.3.0
...
logging.basicConfig(filename="sz_4.log", level=logging.INFO, format=LOG_FORMAT)
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        logging.info("Epoch: %s/%s" %(idx,data.HP_iteration)) # sz --
        ...
        if data.seg:
            current_score = f
            logging.info("Dev: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f"%(dev_cost, speed, acc, p, r, f)) # sz --
        ...
        if data.seg:
            logging.info("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f"%(dev_cost, speed, acc, p, r, f)) # sz --
