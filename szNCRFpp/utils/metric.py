...
def get_ner_fmeasure(golden_lists, predict_lists, label_type="BMES"):
    ...
    for idx in range(0,sent_num):
        ...
        all_tag += len(golden_list) # (本就是词级别)
        if label_type == "BMES":
        ...
        # print(label_type)  # BMES             -- 人为更改
        # print(gold_matrix) # ['[7]1']
        # print(pred_matrix) # ['[1]1', '[7]1'] -- sz
        # print "gold", gold_matrix
        # print "pred", pred_matrix
        right_ner = list(set(gold_matrix).intersection(set(pred_matrix)))
        golden_full += gold_matrix # gold_matrix = [[0,1]PERS,[2]PREP,[3]VERB]
    ...
    # sz 改为计算词的prf
    # print(right_full) ['[2]1', '[7]1']
    # input()
    # print(golden_full) ['[2]1', '[9]1', '[2]1', '[7]1']
    # input()
    # print(predict_full) ['[2]1', '[7]1']
    # input()
    right_num = golden_num = predict_num = 0
    # for a_sent in right_full: right_num += len(a_sent) 
    # for a_sent in golden_full: golden_num += len(a_sent) 
    # for a_sent in predict_full: predict_num += len(a_sent) # 人为改动
    right_num += len(right_full) 
    golden_num += len(golden_full) 
    predict_num += len(predict_full) # 人为改动
...
def get_ner_BMES(label_list):
    ...
    for i in range(0, list_len):
        ...
        current_label = label_list[i].upper() # B- E- S-
    ...
    for i in range(0, tag_list_len):
        if  len(tag_list[i]) > 0:
            ...
            insert_list = reverse_style(tag_list[i]) # PERS[0,1] -> [0,1]PERS
